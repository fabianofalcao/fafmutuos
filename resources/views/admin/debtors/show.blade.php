    ...
    <div class="col-12 col-sm-8 align-items-stretch">
        <div class="card bg-light">
            <div class="card-body">
                <div class="row">
                    <div class="col-7">
                        <h2 class="lead"><b>Nome:</b> {{$register->user->name}}<br/><b>CNPJ:</b> {{$register->user->document}}<br/><b>Mão-de-obra especializada: </b> {{$register->job->description}}<br/><b>Valor do empréstimo:</b> R$ {{$register->value}}</h2>
                        <p class="text-muted text-sm"><b>Motivo do empréstimo: </b> {{($register->source_of_debt)}} <br/> <b>Situação do cliente:</b> {{$register->user->is_active}}</p>
