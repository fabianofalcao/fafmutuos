            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{route('admin.'.$routeName.'.create')}}" class="btn btn-sm btn-success">{{$btnCaption}}</a>
                        <form class="float-right" action="{{route('admin.jobs.search')}}" method="GET">
                            @csrf
                                    ...
                                    @foreach($colunmsTitle as $key => $value)
                                        ...
                                        @else
                                            <td class="text-center align-middle" width="140px">
                                                ...
                                                <form class="form-horizontal" method="POST" action="{{ route('admin.'.$routeName.'.destroy', [$l->id]) }}">
                                                    {{ csrf_field() }}
                                                    @method('DELETE')
                                                    <a href="{{ route('admin.'.$routeName.'.edit', $l->id) }}" class="btn btn-info btn-sm" title="Editar"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                                    <button type="submit" class="btn btn-danger btn-sm" title="Excluir"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                                </form>
                                            </td>
                                        @endif
                                    @endforeach
